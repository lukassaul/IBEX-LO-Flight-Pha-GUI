...
public class ESAInterfaceElectrons  {
	...
	public int n = 60; // at each source location
	...
	public ESAInterfaceElectrons() {
		...
		y=31.5;
		for (int i=0; i<n; i++) {
			x += (42.0-37.0)/(double)n;
			y += (33.4-31.5)/(double)n;
		...
		for (int i=0; i<n; i++) {
			...
			ev = 12000;
			outF.write(0.0+", "+ m2 +", "+ q +", " + (x+1)*gridsPerMM+", " + y*gridsPerMM+", "
							 + 0.0 +", "+az+", "+el+", "+ev+", "+cwf+", "+3 +"\n");
		}
		...
		y=47.3;
		for (int i=0; i<n; i++) {
			x += (37.7-42.2)/(double)n;
			y += (59.4-47.3)/(double)n;
			az = r.nextGaussian()*angleSigma;
			while (Math.abs(az)>maxAngle) {
				az = r.nextGaussian()*angleSigma;
			}
