...
public class GSE_rates_pitch {
	...
	*/
	...
	public GSE_rates_pitch() {
		int numTotal = 0;

		rah = new Histogram(-7.5f,7.5f,15);
		rb0h = new Histogram(-7.5f,7.5f,15);
		rb3h = new Histogram(-7.5f,7.5f,15);
		rch = new Histogram(-7.5f,7.5f,15);
		rt0h = new Histogram(-7.5f,7.5f,15);
		rt1h = new Histogram(-7.5f,7.5f,15);
		rt2h = new Histogram(-7.5f,7.5f,15);
		rt3h = new Histogram(-7.5f,7.5f,15);


		rahN = new Histogram(-7.5f,7.5f,15);
		rb0hN = new Histogram(-7.5f,7.5f,15);
		rb3hN = new Histogram(-7.5f,7.5f,15);
		rchN = new Histogram(-7.5f,7.5f,15);
		rt0hN = new Histogram(-7.5f,7.5f,15);
		rt1hN = new Histogram(-7.5f,7.5f,15);
		rt2hN = new Histogram(-7.5f,7.5f,15);
		rt3hN = new Histogram(-7.5f,7.5f,15);

		//file f = new file("TOF_PHA_20060605_063443.txt");
		file f = new file("h2_pitch_rates_225_16.txt");
		f.initRead();

		// read all appropriate settings from the gui first before reading data file
		outfName = "pitch_scan_h2_15_225_rates_out.txt";
		...
		while ((line=f.readLine())!=null) {
			...
			pitch_val = Float.parseFloat(st.nextToken());
			//time = Long.parseLong(st.nextToken());
			if (roll_val > 360) roll_val = roll_val-65536;
			if (pitch_val > 360) pitch_val = pitch_val-65536;

			//if (time-lastTime==2) {
				for (int i=0; i<ra; i++) rah.addEvent(pitch_val);
				for (int i=0; i<rb0; i++) rb0h.addEvent(pitch_val);
				for (int i=0; i<rb3; i++) rb3h.addEvent(pitch_val);
				for (int i=0; i<rc; i++) rch.addEvent(pitch_val);
				for (int i=0; i<rt0; i++) rt0h.addEvent(pitch_val);
				for (int i=0; i<rt1; i++) rt1h.addEvent(pitch_val);
				for (int i=0; i<rt2; i++) rt2h.addEvent(pitch_val);
				for (int i=0; i<rt3; i++) rt3h.addEvent(pitch_val);

				rahN.addEvent(pitch_val);
				rb0hN.addEvent(pitch_val);
				rb3hN.addEvent(pitch_val);
				rchN.addEvent(pitch_val);
				rt0hN.addEvent(pitch_val);
				rt1hN.addEvent(pitch_val);
				rt2hN.addEvent(pitch_val);
				rt3hN.addEvent(pitch_val);
			//}
			//lastTime  = time;
			numTotal++;
		}
	...
	public static final void main(String[] args) {
	    GSE_rates_pitch ge = new GSE_rates_pitch();
	}
}