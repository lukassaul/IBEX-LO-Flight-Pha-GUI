...
public class GSE_rates_pac {
	...
	*/
	...
	public GSE_rates_pac() {
		int numTotal = 0;

		rah = new Histogram(4000,16000,30);
		rb0h = new Histogram(4000,16000,30);
		rb3h = new Histogram(4000,16000,30);
		rch = new Histogram(4000,16000,30);
		rt0h = new Histogram(4000,16000,30);
		rt1h = new Histogram(4000,16000,30);
		rt2h = new Histogram(4000,16000,30);
		rt3h = new Histogram(4000,16000,30);


		rahN = new Histogram(4000,16000,30);
		rb0hN = new Histogram(4000,16000,30);
		rb3hN = new Histogram(4000,16000,30);
		rchN = new Histogram(4000,16000,30);
		rt0hN = new Histogram(4000,16000,30);
		rt1hN = new Histogram(4000,16000,30);
		rt2hN = new Histogram(4000,16000,30);
		rt3hN = new Histogram(4000,16000,30);

		//file f = new file("TOF_PHA_20060605_063443.txt");
		file f = new file("174045_rates_out2.txt");
		f.initRead();

		// read all appropriate settings from the gui first before reading data file
		outfName = "174045_pac_scan.txt";
		...
		while ((line=f.readLine())!=null) {
			...
			pac_val = Float.parseFloat(st.nextToken());
			mcp_val = Float.parseFloat(st.nextToken());
			time = Long.parseLong(st.nextToken());

			if (time-lastTime==2) {
				for (int i=0; i<ra; i++) rah.addEvent(pac_val);
				for (int i=0; i<rb0; i++) rb0h.addEvent(pac_val);
				for (int i=0; i<rb3; i++) rb3h.addEvent(pac_val);
				for (int i=0; i<rc; i++) rch.addEvent(pac_val);
				for (int i=0; i<rt0; i++) rt0h.addEvent(pac_val);
				for (int i=0; i<rt1; i++) rt1h.addEvent(pac_val);
				for (int i=0; i<rt2; i++) rt2h.addEvent(pac_val);
				for (int i=0; i<rt3; i++) rt3h.addEvent(pac_val);

				rahN.addEvent(pac_val);
				rb0hN.addEvent(pac_val);
				rb3hN.addEvent(pac_val);
				rchN.addEvent(pac_val);
				rt0hN.addEvent(pac_val);
				rt1hN.addEvent(pac_val);
				rt2hN.addEvent(pac_val);
				rt3hN.addEvent(pac_val);
			}
			lastTime  = time;
			numTotal++;
		}
	...
	public static final void main(String[] args) {
	    GSE_rates_pac ge = new GSE_rates_pac();
	}
}